...
function discover_compiler() {
    ...
    local gcc_toolset=gcc-toolset-11
    ...
    if [ -f "/opt/rh/$gcc_toolset/enable" ]; then
        ci_debug "Detected SCL $gcc_toolset environment file"
        compiler_env="/opt/rh/$gcc_toolset/enable"
